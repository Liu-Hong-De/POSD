...
TEST_F(NodeTest, NodeTypeError) {
  ...
  try{   //If the Link doesn't exist, you should throw string "It is not Link!"
    new Link("test/test_folder/hello.txt");
    FAIL();
  } catch(string message) {
...
TEST(FileSystemBuilder, buildFile) {
  FileSystemBuilder * fb = FileSystemBuilder::instance();
  fb->build("test/test_folder/hw1.cpp");
  ASSERT_EQ("hw1.cpp", fb->getRoot()->name());
  ASSERT_EQ("test/test_folder/hw1.cpp", fb->getRoot()->getPath());
}

TEST(FileSystemBuilder, buildFolder) {
  FileSystemBuilder * fb = FileSystemBuilder::instance();
...
TEST(FileSystemBuilder, buildLink) {
  FileSystemBuilder * fb = FileSystemBuilder::instance();
  fb->build("test/test_folder/hw1");
  ASSERT_EQ("hw1", fb->getRoot()->name());
  ASSERT_EQ("test/test_folder/hw1", fb->getRoot()->getPath());
}

TEST(FileSystemBuilder, buildUseMultiple) {
  FileSystemBuilder * fb = FileSystemBuilder::instance();
  fb->build("test/test_folder/hw1.cpp");
  ASSERT_EQ("hw1.cpp", fb->getRoot()->name());
  ASSERT_EQ("test/test_folder/hw1.cpp", fb->getRoot()->getPath());
...
TEST(FileSystemBuilder, buildFolderIterator) {
  FileSystemBuilder * fb = FileSystemBuilder::instance();
  fb->build("test/test_folder");
  ASSERT_EQ("test_folder", fb->getRoot()->name());
  ASSERT_EQ("test/test_folder", fb->getRoot()->getPath());
  ...
  it->next();
  ASSERT_EQ("hw", it->currentItem()->name());
  it->next();
  ASSERT_EQ("hw1", it->currentItem()->name());
  it->next();
  ASSERT_EQ("hw1.cpp", it->currentItem()->name());
  it->next();
  ASSERT_EQ("hw2", it->currentItem()->name());
  it->next();
  ASSERT_EQ("hw2.h", it->currentItem()->name());
  it->next();
  ASSERT_EQ(true, it->isDone());
}
#endif
